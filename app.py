...
def habits():
    ...
    if request.method == 'POST':
        data = request.json
        cursor.execute('''
            INSERT INTO habits (name, habit_type, target_hours, target_type)
            VALUES (?, ?, ?, ?)
        ''', (data['name'], data['habit_type'], data.get('target_hours'), 
              data.get('target_type', 'binary')))
        conn.commit()
...
def habit_logs():
    ...
    if request.method == 'POST':
        data = request.json
        cursor.execute('''
            INSERT INTO habit_logs (habit_id, log_date, hours_spent, completed, completion_percentage, notes)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (data['habit_id'], data['log_date'], data.get('hours_spent'), 
              data.get('completed', False), data.get('completion_percentage'), 
              data.get('notes', '')))
        conn.commit()
        ...
        cursor.execute('''
            SELECT hl.*, h.name, h.habit_type, h.target_hours, h.target_type
            FROM habit_logs hl
...
def edit_habit_log(log_id):
    ...
    if request.method == 'PUT':
        ...
        cursor.execute('''
            UPDATE habit_logs 
            SET hours_spent = ?, completed = ?, completion_percentage = ?, notes = ?
            WHERE id = ?
        ''', (data.get('hours_spent'), data.get('completed', False), 
